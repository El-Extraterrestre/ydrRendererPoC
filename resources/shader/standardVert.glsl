...
void main()
{
    ...
    modelNormal1 = modelMat * normal1NDC;
    modelNormal2 = modelMat * normal2NDC.xyz;

    vec4 mvpPosition = projection * (view * (model * vec4(aPos, 1.0)));

    vertexColor = aColor;
    palColor = texture(tintpalettetex, vec2(aColor.z, 0.0));
    transformedTexCoords = aTex;
    bitangent = cross(modelNormal1, modelNormal2) * normal2NDC.w;
